  ...
  template: `
    <div class="card h-100 border rounded-4 overflow-hidden shadow-sm">
      ...
      <div class="card-body d-flex flex-column p-4 bg-white z-2">
        ...
        <div *ngIf="newBadgeMessage" class="alert alert-success py-2 px-3 small mb-3" role="alert">
          {{ newBadgeMessage }}
        </div>

        <div *ngIf="errorMessage" class="alert alert-warning py-2 px-3 small mb-3" role="alert">
          {{ errorMessage }}
        </div>
        
        <div class="m-0">

          <ng-container *ngIf="status === 'available'; else completedFooter">
            <div class="d-flex justify-content-between align-items-center pt-3 border-top">
              ...
              <button
                type="button"
                class="btn rounded-3 fw-medium px-4"
                [ngClass]="missionData.btnClass"
                [disabled]="isCompleting || justCompleted"
                (click)="completeMission()"
              >
                {{ isCompleting ? 'Completando...' : (justCompleted ? 'Completada' : 'Completar') }}
              </button>
            </div>
          </ng-container>

          <ng-template #completedFooter>
            <div class="d-flex justify-content-between align-items-center pt-3 border-top">
              <span class="text-success fw-semibold small m-0">Mision completada</span>
            </div>
          </ng-template>
...
export class MissionCardComponent {
  ...
  newBadgeMessage: string | null = null;
  ...
  completeMission(): void {
    if (this.isCompleting || this.justCompleted) {
      return;
    }

    this.isCompleting = true;
    this.errorMessage = null;
    this.newBadgeMessage = null;

    let previousBadgeNames: string[] = [];
    this.profileService.badges$.pipe(take(1)).subscribe((badges) => {
      previousBadgeNames = badges.map((b) => b.name);
    });
    ...
    this.missionService.completeMission(this.mission.id).subscribe({
      next: () => {
        ...
        this.profileService.refresh().subscribe(([, badges]) => {
          const newBadge = badges.find((b) => !previousBadgeNames.includes(b.name));
      ...
      error: (err: HttpErrorResponse) => {
        this.isCompleting = false;
        if (err.status === 409) {
          this.errorMessage = 'Ya completaste esta mision';
        } else {
          this.errorMessage = 'Ocurrio un error al completar la mision. Intenta de nuevo.';
        }
      },
    });
  }
