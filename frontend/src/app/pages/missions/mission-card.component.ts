      ...
      <div class="card-body d-flex flex-column p-4 bg-white z-2">
        ...
        <div class="m-0">
          ...
          <ng-template #completedFooter>
            <div class="d-flex justify-content-between align-items-center pt-3 border-top">
              <span class="text-success fw-semibold small m-0">✓ Misión completada</span>
            </div>
          </ng-template>
  ...
  completeMission(): void {
    ...
    this.missionService.completeMission(this.mission.id).subscribe({
      ...
      error: (err: HttpErrorResponse) => {
        this.isCompleting = false;
        if (err.status === 409) {
          this.errorMessage = 'Ya completaste esta misión';
        } else {
          this.errorMessage = 'Ocurrió un error al completar la misión. Intentá de nuevo.';
        }
      },
    });
  }
